...
function updateBasicSettings () {
  ...
  let optionFieldMapD_Init = newDConfigs.map(function (el) { // eslint-disable-line
    return el.field
  })
...
function launchable (event, state) {
  console.log('launchable: ' + state)
  if (state) {
    launchDFbtn.disabled = false
  } else {
    launchDFbtn.disabled = true
  }
}
